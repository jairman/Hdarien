...
function load0(order){
	...
	$('#d_table').load('config_marca.php?o=' + order.replace(/ /g,"+")  + ' #d_table ', 
...
function load1(){
	var o = '';
	$('#d_table').load('config_marca.php?o=' + o.replace(/ /g,"+")  + ' #d_table ', 
	function(response, status, xhr){
		//console.log('r:'+response+' s:'+status+' x: '+xhr);
		//console.log(status);	
		if (status == 'success'){
...
function erase (und){
	overlay.show()
	$("#dialog").html('&nbsp;Eliminar Marca: '+und+"?<br>").css('text-align','center');
...
function createUnd(){
	...
	console.log("und="+und+"&desc="+descp+"&user="+user2+"&action=create_marca")
	
	$.ajax({
		type:"post",
		url:"../controllers/compras_connect.php",
		data:"und="+und+"&desc="+descp+"&user="+user2+"&action=create_marca",				
...
function updateUnd(){
	...
	console.log("und="+und+"&desc="+descp+"&user="+user2+"&action=upd_marca")
	
	$.ajax({
		type:"post",
		url:"../controllers/compras_connect.php",
		data:"und="+und+"&desc="+descp+"&user="+user2+"&action=upd_marca",				
		success:function(data){
			console.log('eu:'+data);
		}
	}).done(updateu = true);
...
function eraseUnd(und){
	var user2 = $('#tf_user2').val();
	console.log("und="+und+"&user="+user2+"&action=del_marca");
	$.ajax({
		type:"post",
		url:"../controllers/compras_connect.php",
		data:"und="+und+"&user="+user2+"&action=del_marca",				
		success:function(data){
			console.log('du:'+data);
		}
	}).done(deleteu = true);
