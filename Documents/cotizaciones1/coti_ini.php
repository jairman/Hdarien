...
<table width="95%" border="1" cellspacing="0" align="center">
    ...
    <div id="menu">
      <ul>
      <li>
      <a href="../cotizaciones/coti_ini.php" class='active'>Cotizaciones</a>
      </li>
      <li>
...
<table width="95%" align="center" border="1" cellspacing="0">
    <tr class="tittle">
    <td colspan="4">Reporte de Cotizaciones</td>
    </tr>
    <tr>
    	...
    	<td class="bold cont" align="right" width="20%">Fecha</td>
        <td class="bold cont" align="right" width="30%"> 
    ...
    <select name="sl_year" id="sl_year" onChange="load2()" style="width:80px" >
		...
		//echo 'yearq';
		if ($ptov_url == ''){
			$query_anos = "SELECT DISTINCT YEAR(f_fact)  FROM `h01sg_cotizaciones` WHERE `delete`<>1
			ORDER BY YEAR(f_fact) DESC";
		}else{
			$query_anos = "SELECT DISTINCT YEAR(f_fact)  FROM `h01sg_cotizaciones` WHERE `delete`<>1 AND `punto_venta`='$ptov_url'
			ORDER BY YEAR(f_fact) DESC";
		}
    ...
    <select name="sl_month" id="sl_month" onChange="load3()" style="width:80px">
		...
		//echo 'monthq';
		if ($ptov_url == ''){
			$query_mes = "SELECT DISTINCT MONTH(f_fact) , MONTHNAME(f_fact) FROM `h01sg_cotizaciones` 
    ...
    <select name="sl_day" id="sl_day" onChange="load4()" style="width:80px">
		...
		//echo 'dayq';
		if ($ptov_url == ''){
			$query_dia = "SELECT DISTINCT DAY(f_fact)  FROM `h01sg_cotizaciones` 
...
<table width="95%" border="1" align="center" cellspacing="0" id="tb_data">
  <tr class="stittle">
  	<td align="center" width="20%" onClick="orden_bus('f_fact')" style="cursor:pointer" title="Ordenar por Fecha">Fecha</td>
    ...
    <td align="center" width="15%" onClick="orden_bus('consec')" style="cursor:pointer" title="Ordenar por No. de Cotización">No</td>
    <td align="center" width="20%" onClick="orden_bus('cliente')" style="cursor:pointer" title="Ordenar por Proveedor">Provedor</td>
    <td align="center" width="10%" onClick="orden_bus('cant')" style="cursor:pointer" title="Ordenar por No. de Items">Items</td>
    <td align="center" width="10%" onClick="orden_bus('costo')" style="cursor:pointer" title="Ordenar por Costo">Costo</td>
  ...
  <?php
	...
	if($year_url=='' && $month_url=='' && $day_url==''){
		//echo 2;
		if ($ptov_url == ''){
			$query_muerte = "SELECT * FROM `h01sg_cotizaciones` WHERE 'delete'<>1 $order_url";
		}else{
			$query_muerte = "SELECT * FROM `h01sg_cotizaciones` WHERE 'delete'<>1 AND `punto_venta`='$ptov_url' $order_url";
		}
	}
	if($year_url!='' && $month_url=='' && $day_url==''){
		//echo 4;
		if ($ptov_url == ''){
			$query_muerte = "SELECT * FROM `h01sg_cotizaciones` WHERE 'delete'<>1 AND YEAR(f_fact) = '$year_url'
			$order_url";
		}else{
			$query_muerte = "SELECT * FROM `h01sg_cotizaciones` WHERE 'delete'<>1 AND YEAR(f_fact) = '$year_url'
	...
	<td align="center" 
	onClick="mostrar('../cotizaciones/coti_fact.php?c=<?php echo $row_muerte['consec'] ?>')">
	...
	src="../img/next.png" width="20" height="20" class="bt_x" style="cursor:pointer" title="Realizar Pedido"
	...
	onClick="mostrar('../cotizaciones/coti_fact.php?c=<?php echo $row_muerte['consec'] ?>')">
	<label id="lb_costo<?php echo $i?>" class="costo"><?php echo number_format($row_muerte['costo'], 2)?></label></td>
	<td align="center" ><label class="red">Pedido</label></td>   
	</tr>
	<?php	
			$i++; 
		}
...
function product_wind(){
	var url = '../cotizaciones/coti_form.php';
	//console.log(url);
	mostrar(url);
...
function regmulti_wind(){
	var url = '../compras/invent_regmulti.php';
	//console.log(url),
	Shadowbox.open({
		content: url,
		player: "iframe",
...
function editar(c){
	overlay.show()
	$("#dialog").html('&nbsp;Editar la Cotización No: '+c+"?<br>").css('text-align','center');
...
function pedir(c){
	overlay.show()
	$("#dialog").html('&nbsp;Realizar pedido de la cotización No: '+c+"?<br>").css('text-align','center');
	$("#dialog").prepend('<img id="theImg2" src="../img/warning.png" width="50" height="50"/>');
	$("span.ui-dialog-title").text('Información Importante').css("text-align", "center"); 
	$("#dialog").dialog("open");
...
function red(c){
	var url = '../cotizaciones/coti_edit.php?c='+c;
	mostrar(url);
}

function blue(c){
	var url = '../pedidos/ped_cot_form.php?c='+c;
	mostrar(url);
}
...
function load0(){
	...
	$('#year').load('coti_ini.php?y=' + y.replace(/ /g,"+") +'&p=' + p.replace(/ /g,"+") +
	'&m=' + m.replace(/ /g,"+") +'&d=' + d.replace(/ /g,"+") +' #year' );
	
	$('#month').load('coti_ini.php?y=' + y.replace(/ /g,"+") +'&p=' + p.replace(/ /g,"+") +
	'&m=' + m.replace(/ /g,"+") +'&d=' + d.replace(/ /g,"+") +' #month ' );
	
	$('#day').load('coti_ini.php?y=' + y.replace(/ /g,"+") +'&p=' + p.replace(/ /g,"+") +
	'&m=' + m.replace(/ /g,"+") +'&d=' + d.replace(/ /g,"+") +' #day' );
	
	$('#table').load('coti_ini.php?y=' + y.replace(/ /g,"+") +'&p=' + p.replace(/ /g,"+") +
