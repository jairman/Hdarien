...
<script src="../js/compras_fact2.js" type="text/javascript"></script>
...
<table width="98%" align="center" id="table_header">
    ...
    <td width="10%" rowspan="3" align="left">
...
<table width="98%" border="1" cellspacing="0" align="center">
  ...
  <tr id="fila_<?php echo $i?>" class="row">
    ...
    <?php 
	mysql_select_db($database_conexion, $conexion);
	$dev = mysql_query("SELECT * FROM `h01sg_compras_devoluciones_detalle` WHERE `consec`='$consec_url' 
	AND `delete`<>1 AND `ref`='$ref' ORDER BY `id`", $conexion) or die(mysql_error());
	$row_dev = mysql_fetch_assoc($dev)
	?>
    <label><?php echo $row_dev['cant_dev'] ?></label>
    </td>
    <td class="cont" align="center">
...
<table width="98%" border="1" cellspacing="0" align="center">
  <tr>
  	<td class="bold" align="center" colspan="4">Total Items: <label id="lb_toti" class="red"><? echo $row_fact['cant']?></label></td>
    
  </tr>
  <tr>
  	<td width="20%" class="bold">Total</td>
    <td width="30%" class="bold"><label id="lb_totc" class="red"><? echo number_format($row_fact['costo'],2)?></label></label></td>
    <td width="20%" class="bold">Total  Nuevo</td>
    <?php 
	mysql_select_db($database_conexion, $conexion);
	$factd = mysql_query("SELECT * FROM `h01sg_compras_devoluciones` WHERE `consec`='$consec_url' AND `delete`<>1 ", $conexion) or die(mysql_error());
	$row_factd = mysql_fetch_assoc($factd);	
	?>
    <td width="30%" class="bold"><label id="lb_totc" class="red"><? echo number_format($row_fact['costo']-$row_factd['total'],2)?></label></td>
  </tr>
  <tr>
