...
function load0(order){
	...
	$('#d_table').load('params_und.php?o=' + order.replace(/ /g,"+")  + ' #d_table ', 
...
function load1(){
	console.log('1');
	var o = '';
	$('#d_table').load('params_und.php?o=' + o.replace(/ /g,"+")  + ' #d_table ', 
	function(response, status, xhr){
		//console.log('r:'+response+' s:'+status+' x: '+xhr);
		//console.log(status);	
		if (status == 'success'){
...
function erase(und){
	overlay.show()
	$("#dialog").html('&nbsp;Eliminar Unidad: '+und+"?<br>").css('text-align','center');
	$("#dialog").prepend('<img id="theImg2" src="../../img/warning.png" width="50" height="50"/>');
	$("span.ui-dialog-title").text('Información Importante').css("text-align", "center"); 
	$("#dialog").dialog("open");
	$("#dialog").append('&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<img id="theImg2" src="../../img/good.png" width="50" height="50" style="cursor:pointer" onclick="eraseUnd(\''+und+'\')"/>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<img id="theImg2" src="../../img/erase.png" width="50" height="50" style="cursor:pointer" onclick="cerrar_dialogo()"/>');
...
function createUnd(){
	...
	console.log("und="+und+"&desc="+descp+"&user="+user2+"&action=create_und")
	
	$.ajax({
		type:"post",
		url:"../controllers/params_connect.php",
		data:"und="+und+"&desc="+descp+"&user="+user2+"&action=create_und",				
...
function updateUnd(){
	...
	console.log("und="+und+"&desc="+descp+"&user="+user2+"&action=upd_und")
	
	$.ajax({
		type:"post",
		url:"../controllers/params_connect.php",
		data:"und="+und+"&desc="+descp+"&user="+user2+"&action=upd_und",				
		success:function(data){
			console.log('eu:'+data);
		}
	}).done(updateu = true);
...
function eraseUnd(und){
	var user2 = $('#tf_user2').val();
	console.log("und="+und+"&user="+user2+"&action=del_und");
	$.ajax({
		type:"post",
		url:"../controllers/params_connect.php",
		data:"und="+und+"&user="+user2+"&action=del_und",				
		success:function(data){
			console.log('du:'+data);
		}
	}).done(deleteu = true);
