...
if (isset($_GET['verifC'])){
	 $id=$_GET['verifC']; 
	 $searchId = mysql_query("SELECT * FROM `d89xz_empleados` WHERE `cedula`='$id' and `esta`<>'no' ") or die(mysql_error()); 
...
if(isset($_POST['insertarC'])){
	...
	$columns = implode("`, `", $ids); 
	$columns = "`".$columns."`"; 
	mysql_query("INSERT INTO d89xz_empleados (".$columns.") VALUES (".$values.")")  or die(mysql_error());
...
if(isset($_POST['verifCA'])){
	$ids=$_POST['verifCA'];
	$vals=$_POST['vals'];
mysql_query("UPDATE  d89xz_empleados set `$ids[0]`='$vals[0]',`$ids[1]`='$vals[1]', `$ids[2]`='$vals[2]', `$ids[3]`='$vals[3]', `$ids[4]`='$vals[4]',`$ids[5]`='$vals[5]',`$ids[6]`='$vals[6]' WHERE id='$vals[7]' ", $conexion) or die(mysql_error().'51');
	print_r($ids);
	print_r($vals);
	return false;
