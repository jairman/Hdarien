var siguiente = new Date().getMonth();
...
function enviarModificarA(){
         ...
         $.get('controlador.php?modificar&'+data, llegadaModificar); 
         $("#dialog").dialog("close"); 
}
...
function guardarA(){
  ...
  if (validarHTML5()){
     if (validarFechasG()){
          $('.modal').fadeIn();
          $('input[type=submit]').attr('disabled',true);
          $.get('controlador.php?registrar=""&'+data, llegadaGuardar);
     }    
   }
   return false;
}
...
function buscarfecha(){
   $.get('controlador.php?fecha=""', evento);
}
...
function verActividades(data,fecha){
   ...
   $.get('controlador.php?verActividad='+data, mostrarActividades);   
}
...
function buscarActividades(data){
   ...
   $('#fecha_i').val(data);
   buscarNotas(data);
   $.get('controlador.php?actividad='+data, actividades);
}
...
function enviarAplazar(){
        $('.modal').fadeIn();
         ...
         $('input[type=submit]').attr('disabled',true);
         $.get('controlador.php?aplazar='+data, llegadaAplazar); 
...
function siEliminarA(data,fecha){
      ...
      mesC = parseInt(elem[1])-1;
      anoC = elem[0];     
      $.get('controlador.php?eliminar='+data+'&fechae='+fecha, llegadaEliminar);
      $("#dialog").dialog("close");
}
...
function siguienteFecha(valor){
   if (valor==1){
      ...
      if(dia<=ultimoDia(mes-1,ano)){
         ...
         $.get('controlador.php?actividad='+fecha, actividades);
      }
      else{
         dia = 1;
         mes = parseInt(elem[1])+1;
...
function cambiarEstadoCumplir(data,id,fecha){
      if (data=='1'){
            $.get('controlador.php?estado='+id, function(data){
             //  console.log(data);
            });
...
function AgragarNotas(){
  if( $('#notaMensaje').text()=="" || $('#notaMensaje').text()=="Escribe aqui tu nota"){
     ...
     var comen = $('#notaMensaje').text();
     $.get('controlador.php?registrar_not='+comen+'&fecha='+fecha, llegadaAgregarNotas);     
   }
}
...
function buscarNotas(data){
   $.get('controlador.php?buscar_nota='+data, llegadaBuscarNotas);     
}
...
function buscarValorNota(id){
   //alert(id);
   $.get('controlador.php?contenido_nota='+id, llegadaBuscarValorNotas); 
}
...
function modificarNota(id){
   //alert(id);
   var comen = $('#notaMensaje').text();
   $.get('controlador.php?modificar_nota='+id+'&comen='+comen, llegadaModificarNotas); 
}
...
function siEliminarNota(id){
   $('id_not').val(id);
   $.get('controlador.php?eliminar_nota='+id, llegadaEliminarNotas);   
}
...
function cargarAnio(){
  //alert($('#lugar_r').val());
   $.get('controlador.php?cargarAnio=""'+'&lugar='+$('#lugar_r').val(), llegadaBuscarAnio);    
}
...
function cargarMeses(){
  //console.log('cargar meses '+ $('#repetido').val());
   $.get('controlador.php?cargarMeses=""'+'&lugar='+$('#lugar_r').val()+'&repetido='+$('#repetido').val(), llegadaBuscarMeses);    
}
...
function cargarDias(){
  //alert('cargar dias');
   if ($('#mes_r').val() == ''){
    ...
    $('#dia_r').html('');
    $('#dia_r').append('<option></option');    
      $.get('controlador.php?cargarDias=""&mes='+ $('#mes_r').val()+'&lugar='+$('#lugar_r').val()+'&repetido='+$('#repetido').val(), llegadaBuscarDias);    
   }
}
...
function buscarLugar(){
 ...
 $.get('controlador.php?buscarLugar=""', llegadaBuscarLugar);   
}
...
function buscarHistorialEventos(parorder,paramOrder){
  order = parorder;
   ...
   $('#repetido').val();
   $.get('controlador.php?buscarHistorial=""&order='+order+'&parmOrder='+paramOrder+'&dia='+dia+'&mes='+mes+'&anio='+ano+'&lugar='+lugar+'&repetido='+$('#repetido').val(), llegadaBuscarHistorial);   
}
...
function buscarResponsable(){
   $.get('controlador.php?responsable=""', llegadaBuscarResponsable); 
}
    ...
    //console.log('Solo puede crear agnda en el presneta año')
     ...
     setTimeout(function () {
       ...
       // window.parent.Shadowbox.close();
        ...
        $.get('controlador.php?registrarRE=""&fecha_i='+fecha+'&actividad='+actividad+'&descripcion='+descripcion+'&hinicio='+hinicio+'&hfin='+hfin+'&lugar='+lugar+'&lugar_re='+punto+'&destino='+destino+'&responsable='+responsable+'&notas='+notas+'&codigo='+codigo, llegadaGuardar);
