...
if (isset($_GET['getCat'])){
	...
	$newcat = mysql_query("SELECT DISTINCT `cat` FROM `h01sg_categoria_prod` WHERE `delete`<>1 ORDER BY `cat` ASC") or die(mysql_error());
	$rows = array();
	
	while($r = mysql_fetch_assoc($newcat)) {
		$ref = $r['cat'];
		$newCat1 = mysql_query("SELECT `cat`, `nombre` FROM `h01sg_categoria_prod` WHERE `cat`='$ref' AND `delete`<>1 ") or die(mysql_error());
...
if (isset($_GET['getMarca'])){
	
	$newund = mysql_query("SELECT DISTINCT `marca` FROM `h01sg_marca_prod` WHERE `delete`<>1 ORDER BY `marca` ASC") or die(mysql_error());
	$rows = array();
	
	while($r = mysql_fetch_assoc($newund)) {
		$ref = $r['marca'];
		$newUnd1 = mysql_query("SELECT `marca`, `nombre` FROM `h01sg_marca_prod` WHERE `marca`='$ref' AND `delete`<>1 ") or die(mysql_error());
	...
	for($j=0;$j<$tama;$j++){
		...
		if ($ref){
			...
			if($det == $inv){
				...
				$updref = mysql_query("UPDATE `h01sg_inventario` SET `cant_ini`='0',`cant_trasl`='0',
				`cant_devo`='0',`cant_vend`='0',`cant_final`='0',`user`='$user',`delete`=1 WHERE `ref`='$ref' 
				AND `punto_venta`='$puntov' AND `delete`<>1  ")or die(mysql_error());
