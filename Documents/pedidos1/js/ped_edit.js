...
function createfact(){
	...
	console.log("consec="+consec+"&fecha="+fecha+"&fdes="+fdes+"&user="+user2+"&prove="+prove+"&tel="+tel+"&ced="+ced+"&obs="+obs+"&costo="+costo+"&cant="+cant+"&orig="+orig+"&cot="+cot+"&action=upd_fact")
	
	$.ajax({
		type:"post",
		url:"../pedidos/ped_connect.php",
		data:"consec="+consec+"&fecha="+fecha+"&fdes="+fdes+"&user="+user2+"&prove="+prove+"&tel="+tel+"&ced="+ced+"&obs="+obs+"&costo="+costo+"&cant="+cant+"&orig="+orig+"&cot="+cot+"&action=upd_fact",				
		success:function(data){
			console.log('f: '+data);
		}
	}).done(ffact = true);	
...
function cuadro(){
	var consec = $('#tf_consec').text()
	overlay.show()
	$("#dialog").html('&nbsp;Actualizar el pedido No: '+consec+"?<br>").css('text-align','center');
