...
<table width="95%" border="0" align="center" cellspacing="0">
  <tr >
    <td width="858" align="left" ><div id="menu">
     <ul >
      <ul>
        <li><a href="../PrincipalCP/clientes/verClientes.php?id=<?php echo $idc_url ?>" >Información del Cliente</a></li>
        <li><a href="../PrincipalCP/clientes/client_histo.php?id=<?php echo $idc_url ?>">Historial de Facturación</a></li>
        <li> <a href="dia_dia_pendiente_cliente.php?id=<?php echo $idc_url ?>"  class='active'>Cuentas Por Cobrar</a> </li>
...
<td  align="center" ><input name="imgb2" type="image"  title="Imprimir" src="../img/imprimir.png" alt=""  width="40" height="40" border="0" align="right" style="cursor:pointer" onclick="imprimir_esto('seleccion2')"/></td>
</tr>
</table>
<table width="99%" border="1" align="center">
<tr>
<td  class="tittle">Cuentas Por Cobrar</td>
</tr>
</table>
 ...
 $usuario_nivel;
	  if($usuario_nivel !='general'){
		      ...
		      if( $usuario_nivel !=''   &&  $f2 =='' ){
	  		 ...
	  		 $query_pen = "SELECT DISTINCT factura,cliente,estado,comentario,hacienda,f_pago,concep,f_alarma ,fecha,consec_fact  FROM d89xz_diario WHERE   estado = 'Pendiente' and  concep='Ingreso' and	hacienda='$usuario_nivel' and cedula='$id'         ORDER BY factura desc";
			  }
	    ...
	    if( $usuario_nivel!='general'   &&  $f2 !='' ){
			...
			$query_pen = "SELECT DISTINCT factura,cliente,estado,comentario,hacienda,f_pago,concep,f_alarma       		    				,fecha,consec_fact  FROM d89xz_diario WHERE      estado = 'Pendiente'   
						 and  concep='Ingreso' and    hacienda='$usuario_nivel' and    fecha >= '$f1'  AND  fecha <='$f2'                   and cedula='$id'       ORDER BY factura desc"; 	
	    ...
	    if($usuario_nivel =='general'){
		  //echo $usuario_nivel;
					...
					//echo "1";
						$query_pen = "SELECT DISTINCT factura,cliente,estado,comentario,hacienda,f_pago,concep,f_alarma ,fecha,consec_fact  FROM d89xz_diario WHERE  estado = 'Pendiente'   
						 and   concep='Ingreso' and   hacienda='$hda'   and    fecha >= '$f1'  AND  fecha <='$f2'  and cedula='$id' ORDER BY factura desc"; 
				  }
				...
				if($hda ==' ' && $f2 !='' ) {
					//echo "2";
						$query_pen = "SELECT DISTINCT factura,cliente,estado,comentario,hacienda,f_pago,concep,f_alarma ,fecha,consec_fact  FROM d89xz_diario WHERE  estado = 'Pendiente'   
						 and  concep='Ingreso' and  fecha >= '$f1'  AND  fecha <='$f2' and cedula='$id'   ORDER BY factura desc"; 
				  }
				...
				// echo "3".$id;
						...
						estado = 'Pendiente'   and concep='Ingreso' and cedula='$row_clien[cedula]' 	ORDER BY factura desc"; 	
					...
					//echo "4".$usuario_nivel;
						$query_pen = "SELECT DISTINCT factura,cliente,estado,comentario,hacienda,f_pago,concep,f_alarma ,fecha,consec_fact  FROM d89xz_diario WHERE      estado = 'Pendiente'   
						 and  concep='Ingreso' and    hacienda='$hda' and cedula='$id'    ORDER BY factura desc"; 	
...
<table width="99%" border="1" align="center" cellspacing="0" id="t_suma">
  ...
  do {
      ...
      <td colspan="2"   align="center" onClick="mostrar('fact.php?c=<?php echo $row_pen['consec_fact'] ?>&p=<?php echo $row_pen['hacienda'] ?>&factura=<?php echo $row_pen['factura'] ?>')" >&nbsp;<?php echo $row_pen['f_alarma']; ?></td>   
...
<td   align="center" ><input name="imgb" type="image" src="../img/mail.png" width="25" height="25" title="Enviar Email"  onClick="mostrar('fact_correo.php?c=<?php echo $row_pen['consec_fact'] ?>&p=<?php echo $row_pen['hacienda'] ?>&factura=<?php echo $row_pen['factura'] ?>')" /></td>
    </tr>
        <?php 
...
//	alert ('hda')
	...
	$('#month').load('dia_dia_pendiente_cliente.php?h=' + h.replace(/ /g,"+") + '&y=' + y.replace(/ /g,"+") +
	'&m=' + m.replace(/ /g,"+")  +' #month ' );
	

	
	$('#seleccion11').load('dia_dia_pendiente_cliente.php?h=' + h.replace(/ /g,"+") + '&y=' + y.replace(/ /g,"+") +
...
function load3(){
	...
	$('#table').load('dia_dia_pendiente_cliente.php?h=' + h.replace(/ /g,"+") + '&y=' + y.replace(/ /g,"+") +
