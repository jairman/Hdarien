...
<form id="form1" name="form1" method="post" action="">
  <table width="456" border="1" align="center" cellspacing="0">
    <tr>
      <th colspan="4" bgcolor="#4D68A2"><p>Asignar Jornada de Vacunación</p></th>
      </tr>
    <tr>
      <td width="180" style="font-weight: bold">Fecha De Jornada</td>
      <td width="260" colspan="3"> D<span id="spry_dia">
...
?>
    ...
    <tr>
      ...
      <td colspan="3"><span id="spryselect7">
        <select name="sexo" id="sexo" style="width:250px">
          <option>Sexo</option>
...
?>
    ...
    <tr>
      <th colspan="4"><label for="raza"></label>        <label for="color"></label>        <label for="clase"></label>        <label for="sexo"></label>
        ...
        <a  href="diario_pendientes.php" onClick="javascript:window.parent.Shadowbox.close();"><img src="cancelar.png" alt="" width="68" height="20" /></a> </a></span>
...
$insertar = mysql_query("INSERT INTO d89xz_tareas(`tarea`,`fecha_ini`,`fecha`,`estado`,jorn,hac,respon,comen) VALUES ('{$textarea_tarea}',NOW(),'{$text_f_tarea}','{$estado}','{$text_f_tarea}','{$ubica}','{$responsable}','Jornada Vacunacion')",$conexion);

	


$insertar = mysql_query("UPDATE `d89xz_vacunos` SET `vacuna`='$text_f_tarea', `resvac`= '$responsable', `cmvac`='$textarea_tarea' WHERE `raza`='$raza' and `color`='$color' and `sexo`='$sexo' and `clase`='$clase' and `hierro`='$hierro' and ubicasion='$ubica' ", $conexion);
