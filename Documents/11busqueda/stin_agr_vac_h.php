...
<ul id="MenuBar1" class="MenuBarHorizontal">
  ...
  <li onclick="llamarhierro()"><a   class="current">Hierro</a></li>
  <li onclick="llamarclas()"><a>Clasificaci&oacute;n</a>  </li>
</ul>
    <p>&nbsp;</p>
<p>&nbsp;</p>
...
<p>&nbsp;</p>
    <div id="apDiv2">
      <span id="spryselect2" >
        <select name="bus_hierro" id="buscar_hierro"  <?php if($d){ ?>value="<?php echo $d ?>" <?php } ?> style="width:150px; border:#999; text-align:center" >
          <option value="value">Seleccionar</option>
          <?php
do {  
?>
          <option value="<?php echo $row_pesos['hierro']?>"><?php echo $row_pesos['hierro']?></option>
          <?php
} while ($row_pesos = mysql_fetch_assoc($pesos));
  $rows = mysql_num_rows($pesos);
  if($rows > 0) {
      mysql_data_seek($pesos, 0);
	  $row_pesos = mysql_fetch_assoc($pesos);
  }
?>
      </select>
...
<div id="recargar2">
    ...
    <?
		  if($d){
			  $sol= $_GET['sol'];
	     mysql_select_db($database_conexion, $conexion);
		$queEmp ="SELECT * FROM  d89xz_vacunos where hierro LIKE '%$d%' and vendido=''" ;
		$resEmp = mysql_query($queEmp, $conexion) or die(mysql_error());
		$totEmp = mysql_num_rows($resEmp);
		
		  
		?>
        <table width="376"  border="1" align="center" id="mos_t_hie">
          <tr align="center" >
...
$('#buscar_hierro').change(function(){
	var solicitud = getUrlVars()["solicitud"];
	$('#recargar2').html("Cargando...");
	$('#recargar2').load('stin_agr_vac_h.php?hierro='+$('#buscar_hierro').val().replace(/ /g,"+")+ '&sol=' + solicitud +' #hijo2 ');
		});
function getUrlVars() {
    var vars = {};
...
function eliminar(hola, hierro) {	
   var tabla = document.getElementById("mos_t_hie");
