...
<p>&nbsp;</p>
<ul id="MenuBar1" class="MenuBarHorizontal">
    <li><a href="bus_detalle_dia_dia.php" class="current">Reportes De Ventas</a>  </li>
...
<form id="form1" name="form1" method="post" action="">
  <table width="100%" border="1" align="center" cellspacing="0">
    <tr>
      <th colspan="3" bgcolor="#4D68A2"><span style="color: #FFF">Reporte  De Ventas</span></th>
    </tr>
    <tr>
...
if($dia != ''){
	...
	$result = mysql_query("SELECT SUM(`v_tal`) as total FROM d89xz_diario where YEAR(fecha) = '$anos' AND MONTH(fecha) = '$mes' AND DAY(fecha) = '$dia' AND concep = 'Ingreso'"); 
...
$queEmp ="SELECT * FROM d89xz_diario where  YEAR(fecha) = '$anos' AND MONTH(fecha) = '$mes' AND concep = 'Ingreso' and DAY(fecha) = '$dia'";
		...
		echo "</tr> \n";
					...
					if ($totEmp> 0) {
						...
						@$v_tal = number_format ($rowEmp['v_tal']);
...
$result = mysql_query("SELECT SUM(`v_tal`) as total FROM d89xz_diario where YEAR(fecha) = '$anos' AND MONTH(fecha) = '$mes' AND concep = 'Ingreso' "); 
...
@$mensual =  number_format($row1[total]);
	...
	echo "<table border = '1' width='100%' cellspacing='0'> \n";
				...
				echo "</tr> \n";
					...
					$queEmp ="SELECT * FROM d89xz_diario where YEAR(fecha) = '$anos' AND MONTH(fecha) = '$mes' AND concep = 'Ingreso' ";
					$resEmp = mysql_query($queEmp, $conexion) or die(mysql_error());
					$totEmp = mysql_num_rows($resEmp);
...
if(($dia == 0) &&  ($mes == 0)){
	...
	$result = mysql_query("SELECT SUM(`v_tal`) as total FROM d89xz_diario where YEAR(fecha) = '$anos'  AND concep = 'Ingreso'"); 
 
	$row = mysql_fetch_array($result, MYSQL_ASSOC);
	
	@$anual =  number_format($row[total]);
	...
	echo "<table border = '1' width='100%' cellspacing='0'> \n";
				...
				echo "</tr> \n";
					...
					$queEmp ="SELECT * FROM d89xz_diario where YEAR(fecha) = '$anos' AND concep = 'Ingreso' order by fecha";
					$resEmp = mysql_query($queEmp, $conexion) or die(mysql_error());
					$totEmp = mysql_num_rows($resEmp);
