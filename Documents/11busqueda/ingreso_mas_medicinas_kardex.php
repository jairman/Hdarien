...
<table width="100%" border="0" align="center" cellspacing="0">
  <tr bgcolor="#f0f0f0">
    <td width="121" align="left"><a rel="shadowbox[ejemplos];options={continuous:true}" href="editar_ingre_medicinas.php?id=<?php echo $id ?>&amp;tipo=<?php echo $tipo?>&amp;nombre=<?php echo $nombre?>&amp;decrip=<?php echo $descrip?>&amp;coment=<?php echo $coment?>&amp;contenido=<?php echo $cont?>&amp;marca=<?php echo $mark?>"><img src="modificar.png" alt="" width="68" height="20" /></a></td>
    <td width="121" align="left"><a onclick="return confirm('Realmente Desea Eliminar La Medicina');" href="eliminar_medicinas.php?id=<?php echo $id ?>&amp;tipo=<?php echo $tipo?>&amp;nombre=<?php echo $nombre?>&amp;decrip=<?php echo $descrip?>&amp;coment=<?php echo $coment?>&amp;contenido=<?php echo $cont?>&amp;marca=<?php echo $mark?>"><img src="eliminar.png" alt="" width="68" height="20" /></a></td>
    <td width="308" align="center"><a href="kardex_medicina.php"><img src="last.png" alt="" width="29" height="31" border="0" /></a></td>
...
<table width="710" border="1" align="center" cellspacing="0">
  ...
  <tr>
    <th colspan="9" bgcolor="#4D68A2"><p class="x">Ingreso de  Medicinas</p></th>
    </tr>
  <tr>
    ...
    <td width="62">Concepto</td>
    <td width="103"><span id="spryselect2">
...
?>
    ...
    <th width="62"><input type="submit" name="button" id="button" value="Enviar" /></th>
  </tr>
</table>
</form>
<p>&nbsp;</p>
...
<?
	...
	@$dosis= $text_cantidad * $select_contenido;
		...
		$insertar = mysql_query("INSERT INTO d89xz_total_medicinas_salidas (tipo,nombre,cont,mark,cantid,fecha,concep,idm)
		VALUES ('{$select_tipo}','{$select_nombre}','{$select_contenido}','{$select_descrip}','{$text_cantidad}','{$text_f_jornada}','{$entrada}','{$id}')", $conexion);
				
		
$insertar1 = mysql_query("UPDATE  `d89xz_total_medicinas` SET `cantid`=cantid + $text_cantidad, `dosis`= dosis + $dosis WHERE `id` = '$id'", $conexion);
...
echo $dosis;
		...
		$insertar1 = mysql_query("INSERT INTO d89xz_total_medicinas_salidas (tipo,nombre,cont,mark,cantid,fecha,concep,idm)
		VALUES ('{$select_tipo}','{$select_nombre}','{$select_contenido}','{$select_descrip}','{$text_cantidad}','{$text_f_jornada}','{$entrada}','{$id}')", $conexion);
		
$insertar = mysql_query("UPDATE  `d89xz_total_medicinas` SET `cantid`=`cantid`- $text_cantidad, `dosis`= dosis - '$dosis' WHERE `id` = '$id'", $conexion);
