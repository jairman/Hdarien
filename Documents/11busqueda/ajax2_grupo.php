...
<table width="100%" border="0" align="center" cellspacing="0">
  <tr>
    <td colspan="4" align="center" bgcolor="#f0f0f0"><a href="diario_pendientes.php"><img src="last.png" alt="" width="29" height="31" border="0" /></a></td>
  </tr>
  <tr>
...
<form action="" method="post" enctype="application/x-www-form-urlencoded" name="form1" id="form1">
  ...
  <tr>
    <th colspan="4" bgcolor="#4D68A2">Ingrese los datos de la Jornada</th>
    </tr>
  <tr>
    <td width="337"><span style="width:100px; float:left">
  ...
  ?>
    ...
    <td width="355"><label for="textfield">
    ...
    <th width="164"><span id="sprytextfield2">
      <label for="distrito"></label>
      <input name="distrito" type="text" id="distrito" size="15">
    </span></th>
  </tr>
  <tr>
    <td><span style="width:150px; float:left">Observaciones :</span></td>
    <td colspan="2"><span style="width:150px; float:left">
      <textarea name="textarea_observ" id="textarea_observ" cols="35" rows="1"></textarea>
      </span></td>
...
@$total = $select_tratamiento * $cuenta;
...
if($select_tratamiento != ''){
			
$insertar3 = mysql_query("UPDATE `d89xz_vacunasion` SET `jornada`= '$jornada', `diagnostico`= '$select_diagnostico', `tratamiento`= '$select_tratamiento', `observasion`= '$textarea_observ', `fecha`= NOW(), `fito`= 'Fito_Sanitaria' WHERE `respon` = '$resvac' and comen='$cmvac' and fito='' ", $conexion);
		
		$insertar1 = mysql_query("UPDATE  `d89xz_total_medicinas` SET `dosis`= dosis - '$total' WHERE `tipo` = '$jornada' AND `nombre` = '$select_diagnostico' and cont='$conte' and mark='$marca'", $conexion);
		
$insertar2 = mysql_query("UPDATE `d89xz_vacunos` SET `vacuna`='',`cmvac`='',`resvac`=''   WHERE `cmvac` ='$cmvac' and `resvac` = '$resvac' and `vacuna` ='$vacuna'", $conexion);

$insertar = mysql_query("INSERT INTO d89xz_total_medicinas_salidas (concep,fecha,cantid,mark,cont,nombre,tipo,idm)
		VALUES ('Jornada Vacunacion','{$vacuna}','{$total}', '{$marca}', '{$conte}','{$select_diagnostico}','{$jornada}','{$id_dep}')", $conexion);	
			
echo "<script type=''>
		window.location='diario_pendientes.php';
	</script>";					
...
var sprytextfield2 = new Spry.Widget.ValidationTextField("sprytextfield2", "integer", {isRequired:false, validateOn:["blur"]});
</script>
