...
if (isset($_GET['getref'])){
	...
	$getRef = mysql_query("SELECT * FROM `h01sg_inventario` WHERE `delete`<>1 AND `punto_venta`='$orig' 
	AND `ref`='$ref' AND `cant_final`>0 ") or die(mysql_error());
...
if (isset($_GET['getinfo'])){
	...
	if ($tipo == 'Mayor'){
		$getInfo = mysql_query("SELECT  `ref`, `fecha`, `cod_barra`, `rfid`, `marca`, `desc`, `precio_mayo` as precio, `costo_und` 
	FROM `h01sg_producto` WHERE `ref`='$ref' AND `delete`<>1 ") or die(mysql_error());
	}else{
		$getInfo = mysql_query("SELECT  `ref`, `fecha`, `cod_barra`, `rfid`, `marca`, `desc`, `precio_und` as precio, `costo_und` 
	FROM `h01sg_producto` WHERE `ref`='$ref' AND `delete`<>1 ") or die(mysql_error());
	}
...
if ($action == "new_client"){
	...
	('$ced','$nombre','$tel','$cumple','$dir','$email','$user')") or die(mysql_error());
