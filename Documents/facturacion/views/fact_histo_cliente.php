...
<script src="../js/fact_histo_cliente.js" type="text/javascript"></script>
...
<table width="95%" align="center" id="table_header">
  <tr>
    ...
    <div id="menu">
      ...
      <li>
      <a  href="fact_crehisto.php?r=<?php echo $ref_url ?>" class='active' >Reporte de Creditos</a>
      </li>
      </ul>
    </div>  
    </td>
    <td width="8%" align="left"><img  title="Graficas" src="../../img/chart.png" alt="" 
    width="48" height="48" border="0" align="right" style="cursor:pointer" onclick="graficas()"/></td>
    <td width="8%" align="left"><img  title="Imprimir" src="../../img/imprimir.png" alt="" 
...
<table width="95%" align="center" id="table_detail">
    <tr class="stittle">
      <td width="12%" onClick="orden_bus('fecha')" style="cursor:pointer" title="Ordenar por Fecha">Fecha</td>
      <td width="13%" onClick="orden_bus('consec')" style="cursor:pointer" title="Ordenar por No de Factura">Consecutivo</td>
      <td width="12%" onClick="orden_bus('forma_pago')" style="cursor:pointer" title="Ordenar por Forma de Pago">Forma</td>
      <td width="15%" onClick="orden_bus('cliente')" style="cursor:pointer" title="Ordenar por Nombre">Cliente</td>
      <td width="13%" onClick="orden_bus('ced')" style="cursor:pointer" title="Ordenar por NIT">NIT</td>
      <td width="13%" onClick="orden_bus('punto_venta')" style="cursor:pointer" title="Ordenar por Punto de Venta">Origen</td>
      <td width="11%" onClick="orden_bus('total_items')" style="cursor:pointer" title="Ordenar por Cantidad de Articulos">Articulos</td>
      <td width="11%" onClick="orden_bus('valor_tot')" style="cursor:pointer" title="Ordenar por Valor de la Venta">Total</td> 
	...
	if($year_url=='' && $month_url=='' && $day_url==''){
		//echo 1;
		if ($ptov_url == ''){
			$query_prod = "SELECT * FROM `h01sg_venta` WHERE `delete`<>1 $order_url";
		}else{
			$query_prod = "SELECT * FROM `h01sg_venta` WHERE `delete`<>1 AND `punto_venta`='$ptov_url' $order_url";
		}
	}
	if($year_url!='' && $month_url=='' && $day_url==''){
		//echo 3;
	...
	while($row_prod = mysql_fetch_assoc($prod)){
		...
		*/
		...
		if ($estado == 0 && $fpago == 'Contado'){
	?>
    <tr class="row" onClick="mostrar('../views/fact.php?c=<?php echo $consec ?>&p=<?php echo $ptov ?>')">
      <td align="center" ><?php echo $row_prod['fecha']?></td>
      <td align="center" ><?php echo $consec?></td>
      <td align="center" ><?php echo $row_prod['forma_pago']?></td>
    ...
    <tr class="row" onClick="mostrar('../views/fact.php?c=<?php echo $consec ?>&p=<?php echo $ptov ?>')">
      ...
      <td align="center" ><?php echo $row_prod['total_items']?></td>
      <td align="center" >
      <label ><?php echo number_format($row_prod['valor_tot'], 2)?></label>
      <input type="hidden" value="c_<?php echo $consec?>">
      </td>
    </tr>
    <?php 
		$i++;
		...
		if ($estado == 2){
	?>
    <tr class="row" onClick="mostrar('../views/factd.php?c=<?php echo $consec ?>&p=<?php echo $ptov ?>')">
      <td align="center" ><?php echo $row_prod['fecha']?></td>
      <td align="center" ><?php echo $consec?></td>
      <td align="center" ><?php echo $row_prod['forma_pago']?></td>
      <td align="center" ><?php echo $row_prod['cliente']?></td>
