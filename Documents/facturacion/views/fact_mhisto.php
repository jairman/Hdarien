...
<head>
<meta charset="UTF-8">
<title>Historial de Ventas por Marca</title>
...
<script src="../js/fact_mhisto.js" type="text/javascript"></script>
...
<table width="95%" align="center" id="table_header">
  <tr>
    ...
    <div id="menu">
      <ul>
      <li>
      	<a  href="fact_histo.php" >Historial de Ventas</a>
      </li>
      <li>
      	<a  href="fact_mhisto.php" class='active'>Historial de Ventas por Marca</a>
      </li>
      </ul>
    </div>  
    </td>
...
<table width="95%" align="center">
	...
	<td class="bold cont" align="right" width="20%">Marca</td>
    <td class="bold cont" align="right" width="30%">
    <select name="sl_marca" id="sl_marca" class="long">
      <option value="">Seleccione</option>
		<?php
        mysql_select_db($database_conexion, $conexion);
        $query_clr = "SELECT DISTINCT `marca` FROM `h01sg_producto` ORDER BY `marca` ASC ";
...
<table width="95%" align="center" id="table_detail">
	...
	if($year_url=='' && $month_url=='' && $day_url==''){
		//echo 1;
		if ($ptov_url == ''){
		$query_prod = "SELECT * FROM `h01sg_venta_detalle` WHERE `ref`='$ref_url' AND `delete`<>1 ORDER BY `fecha` ASC ";
		}else{
			$query_prod = "SELECT * FROM `h01sg_venta_detalle` WHERE `ref`='$ref_url' AND `delete`<>1 AND `punto_venta`='$ptov_url' ORDER BY `fecha` ASC ";
		}
	}
	if($year_url!='' && $month_url=='' && $day_url==''){
		//echo 3;
	...
	if($year_url!='' && $month_url!='' && $day_url!=''){
		...
		AND YEAR(fecha) = '$year_url' AND MONTH(fecha) = '$month_url' AND DAY(fecha) = '$day_url' ORDER BY `fecha` ASC";
			...
			$query_prod = "SELECT * FROM `h01sg_venta_detalle` WHERE `ref`='$ref_url' AND `delete`<>1
		AND YEAR(fecha) = '$year_url' AND MONTH(fecha) = '$month_url' AND DAY(fecha) = '$day_url' AND `punto_venta`='$ptov_url' ORDER BY `fecha` ASC";
		}
