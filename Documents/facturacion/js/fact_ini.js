	...
	if (fdiario == true && ffact == true && iteracionesb >= iteraciones){
		...
		setTimeout(function () {
			...
			if (no == '1'){
				...
				var url = '../views/fact2.php?c='+consec+'&p='+orig;
				window.location.href = url; 
			}
			console.log('1000ms');
		}, 1000);		
...
function control(i){
	...
	var auxSearch = "";
	var cont =0;
	$('#tf_ref'+i).keydown(function(e) {
		cont ++; 
		search = $(this).val();
		len = search.length;	
		if(cont==1){	
			setTimeout(function () {
			var user = $('#tf_ref'+i).val();
...
function agregarFila(){
	...
	var string = '';
	//onFocus="control('+i+')"
	string = string + '<tr id="fila_'+i+'">';
	string = string + '<td class="cont"><input name="tf_ref'+i+'" type="text" id="tf_ref'+i+'" style="width:70%" class="ref" required><img src="../../img/search.png" alt="Busqueda" name="bt_sproduct'+i+'" width="20" height="20" id="bt_sproduct'+i+'" style="cursor:pointer" title="Busqueda" onClick="searchProd('+i+')" /></td>';
	string = string + '<td class="cont"><input type="text" id="lb_det'+i+'" class="long" readonly></td>';
	string = string + '<td class="cont"><input name="tf_cant'+i+'" type="text" id="tf_cant'+i+'" class="long cant" onkeyup="checkNum(this),compare('+i+'),total('+i+')" required ><input type="hidden" id="tf_cantmax'+i+'"></td>';
	string = string + '<td class="cont"><input type="text" id="lb_val'+i+'" class="long" readonly ><input type="hidden" id="tf_valm'+i+'" /></td>';
	string = string + '<td class="cont"><input type="text" id="tf_dcto'+i+'" onkeyup="getdcto('+i+'),total('+i+')" class="long"></td>';
	string = string + '<td class="cont"><input type="text" id="lb_totprod'+i+'" class="long" readonly></td>';
...
function createfact(){
	...
	//console.log("consec="+consec+"&fecha="+fecha+"&user="+user2+"&ptov="+orig+"&tel="+tel+"&ced="+ced+"&nombre="+nombre+"&items="+items+"&total="+total+"&subtotal="+subtotal+"&iva="+iva+"&dcto="+dcto+"&fechap="+fechap+"&tipo="+tipo+"&formap="+formap+"&d="+d+"&action=new_fact");
	
	$.ajax({
		type:"post",
		url:"../controllers/fact_connect.php",
		data:"consec="+consec+"&fecha="+fecha+"&user="+user2+"&ptov="+orig+"&tel="+tel+"&ced="+ced+"&nombre="+nombre+"&items="+items+"&total="+total+"&subtotal="+subtotal+"&iva="+iva+"&dcto="+dcto+"&fechap="+fechap+"&tipo="+tipo+"&formap="+formap+"&d="+d+"&action=new_fact",				
		success:function(data){
			//console.log('f: '+data);
		}
	}).done(ffact = true);
...
function envia(j){
	...
	$.ajax({
		...
		dataType: "json",
		data: {vals2: regs, j:i, tam_env:tam_env, consec:consec, fecha:fecha, user:user2, ptov:orig},
...
function searchClient(){
	var url = '../views/fact_csearch.php';
	Shadowbox.open({
		content: url,
		player: "iframe",
		options: {                   
